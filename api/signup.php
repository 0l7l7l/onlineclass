...
if (!$username || !$email || !$name || !$password) {
    bad('필수 항목을 모두 입력해 주세요.');
}
if (!$agree) {
    bad('약관 및 개인정보 처리방침에 동의해야 회원가입이 가능합니다.');
}
//if (!preg_match('/^[A-Za-z0-9]{4,50}$/', $username)) { bad('아이디는 영문, 숫자 4~50자로 입력해 주세요.');}
if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    bad('이메일 형식이 올바르지 않습니다.');
}
if (strlen($password) < 8) {
    bad('비밀번호는 최소 8자 이상이어야 합니다.');
}
...
try {
 ...
    // 바인딩 파라미터 순서  확인 (총 7개)
    // 1: username, 2: password(hash), 3: name, 4: phone_number, 5: email, 6: consent_version, 7: consent_ip
    $ins->execute([
        $username, 
        $hash, 
        $name, 
