...
function isTeacherRole(?string $role): bool {
    $role = trim((string)$role);
    return strcasecmp($role, 'TEACHER') === 0;
}
...
// - /api/login.php?action=session&type=TEACHER
// ============================================================
if ($method === 'GET' && $action === 'SESSION') {
    ...
    if ($requestedType === 'TEACHER' && !isTeacherRole($sessionUserRole)) {
        jsonResponse(200, ['success' => false, 'user' => null]);
    }
...
// teachadmin/teachers.html -> type=TEACHER
// ============================================================
...
try {
    ...
    if ($loginType === 'ADMIN') {
        ...
        // 강사용 로그인은 TEACHER만 허용
        if (!isTeacherRole($dbRole)) {
            jsonResponse(403, ['success' => false, 'message' => '강사(선생님) 계정으로만 로그인할 수 있습니다.']);
        }
    } else {
        // 학생 페이지 로그인은 STUDENT만 허용
        if (!isStudentRole($dbRole)) {
    ...
    $responseData = [
        'success' => true,
        'message' => $loginType === 'ADMIN' ? '관리자 로그인에 성공했습니다.' : ($loginType === 'TEACHER' ? '강사 로그인에 성공했습니다.' : '로그인에 성공했습니다.'),
        'user' => [
